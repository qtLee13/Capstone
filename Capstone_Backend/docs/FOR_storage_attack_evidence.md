...
## ✅ สูตรคิดคะแนน — ยกให้ฝั่งพี่แล้ว ทำเรียบร้อย

เดิมฝั่งเรามี `risk_score.py` ที่ `WEIGHT_AI = 0.35` ขัดกับ `AI_WEIGHT = 0.20` ของพี่
= อีเมลฉบับเดียวกันได้คนละคะแนนแล้วแต่ว่าใครคิด

**สรุปแล้วว่าเป็นของฝั่งพี่ ฝั่งเราลบทิ้งแล้ว** (2026-08-26)

ตอนไล่โค้ดเจอว่าจริง ๆ สูตรฝั่งเรา **ไม่ถูกเรียกจากที่ไหนเลยอยู่แล้ว** —
`/analyze` เปลี่ยนมาตอบเฉพาะ `raw_signals` ตั้งแต่ย้ายเครื่อง
`compute_final_score()` เลยค้างเป็น dead code รอสร้างความสับสน ตอนนี้ลบออกหมดแล้ว
เหลือแค่ประตู fast path (ตัดสินว่าจะยิง VirusTotal/IPQS ไหม — ไม่ใช่คะแนนความเสี่ยง)

```
AI (เรา)             .92 (พี่)                  Gateway / Mail server
สัญญาณดิบ      →     คิดคะแนน + ระดับ      →     forward / quarantine / block
```

**เส้นแบ่งจากนี้:** ฝั่งเราไม่ตัดสินว่า "เสี่ยงแค่ไหน" อีกแล้ว หน้าที่เราคือส่งสัญญาณให้ครบและตรง
ถ้าพี่อยากได้สัญญาณเพิ่ม บอกได้เลย แต่การถ่วงน้ำหนักเป็นของพี่ล้วน

> ⚠️ พร้อมกันนี้ `INTEGRATION.md` ก็แก้แล้ว — ของเดิมยังเขียนว่า `/analyze` ตอบ
> `summary.final_risk_score` ซึ่งเซิร์ฟเวอร์เลิกตอบไปนานแล้ว ใครเขียนตามเอกสารเก่าจะเจอ KeyError
